...
export const metadata = {
  title: "Tarjetas — Liga",
  description: "Tabla de jugadores con tarjetas por categoría",
}
...
async function TarjetasContent({ categoryId, leagueId }: { categoryId?: string; leagueId?: string }) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Jugadores amonestados y expulsados por categoría
          </p>
        </div>
        <div className="flex flex-col items-end gap-2">
          <LeagueSelector leagues={leagues} />
          <CategorySelector categories={categories} basePath="/tarjetas" />
        </div>
      </div>
                ...
                <TableRow className="bg-muted/50">
                  ...
                  <TableHead className="w-28 text-center">
                    <span className="inline-flex items-center gap-1.5 justify-center">
                      <span className="inline-block h-3 w-2 rounded-sm bg-yellow-400 ring-1 ring-yellow-400/50" />
                      Amarillas
                    </span>
                  </TableHead>
                  <TableHead className="w-24 text-center">
                    <span className="inline-flex items-center gap-1.5 justify-center">
                      <span className="inline-block h-3 w-2 rounded-sm bg-red-500 ring-1 ring-red-500/50" />
                      Rojas
                    </span>
                  </TableHead>
              ...
              <TableBody>
                {cardsData.map((player, idx) => (
                  ...
                  >
                    ...
                    <TableCell className="text-center">
                      <span className="inline-flex items-center justify-center rounded-full bg-orange-100 px-3 py-1 text-sm font-bold text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">
                        {player.totalCards}
                      </span>
                    </TableCell>
                  </TableRow>
...
export default async function TarjetasPage({
  ...
  return (
    ...
    >
      <TarjetasContent categoryId={categoryId} leagueId={leagueId} />
    </Suspense>
  )
}
