...
import {
  ArrowLeft, Users, Swords, Goal, Trophy, Hash, Calendar, Shirt, AlertTriangle,
} from "lucide-react"
...
import { TeamVsSelector } from "@/components/public/team-vs-selector"
...
const resultLabel: Record<string, string> = { W: "G", D: "E", L: "P" }

export default async function LeagueTeamDetailPage({ params }: Props) {
  ...
  const categoryTeams = await db.team.findMany({
    where: { categoryId: team.categoryId, id: { not: id } },
  ...
  const stats = {
    pts: s?.pts ?? 0, pj: s?.pj ?? 0, pg: s?.pg ?? 0, pe: s?.pe ?? 0,
    pp: s?.pp ?? 0, gf: s?.gf ?? 0, gc: s?.gc ?? 0, dg: s?.dg ?? 0,
    ta: s?.ta ?? 0, tr: s?.tr ?? 0,
  }
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-heading text-2xl font-bold tracking-tight md:text-3xl">{team.name}</h1>
            <p className="mt-1 text-muted-foreground">
              {team.category.name} &mdash; {team.category.league.name}
            </p>
          </div>
        </div>
        <Badge variant="secondary" className="text-sm px-3 py-1">{team.shortName}</Badge>
      </div>
          ...
          <Card key={item.label} size="sm" className="text-center">
            <CardContent className="px-2 py-3">
              ...
              <div className={cn(
                ...
                {item.sign ? (item.value > 0 ? `+${item.value}` : item.value) : item.value}
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <section>
        <div className="mb-4 flex items-center gap-2">
          ...
          <span className="rounded-full bg-muted px-2.5 py-0.5 text-xs text-muted-foreground">{team.players.length}</span>
        </div>
        {team.players.length === 0 ? (
          <EmptyState icon={Users} title="Sin jugadores activos" description="Este equipo no tiene jugadores activos en este momento." />
        ) : (
          <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50">
                  <TableHead className="w-16 text-center"><Hash className="mx-auto h-4 w-4" /></TableHead>
                  <TableHead>Nombre</TableHead>
                  <TableHead className="hidden sm:table-cell">Apellido</TableHead>
                  <TableHead className="w-14 text-center">G</TableHead>
                  <TableHead className="w-14 text-center">T</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {team.players.map((player) => (
                  <TableRow key={player.id} className="transition-colors hover:bg-muted/40">
                    <TableCell className="text-center font-mono text-sm font-bold text-muted-foreground">{player.jerseyNumber ?? "—"}</TableCell>
                    <TableCell className="font-medium">
                      <Link href={`/liga/${slug}/jugadores/${player.id}`} className="hover:underline transition-colors">{player.name}</Link>
                    </TableCell>
                    <TableCell className="hidden text-muted-foreground sm:table-cell">
                      <Link href={`/liga/${slug}/jugadores/${player.id}`} className="hover:underline transition-colors">{player.surname}</Link>
                    </TableCell>
                    <TableCell className="text-center font-bold text-primary">{player._count.goals}</TableCell>
                    <TableCell className="text-center text-muted-foreground">{player._count.cards}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      <section>
        <div className="mb-4 flex items-center gap-2">
          ...
          <span className="rounded-full bg-muted px-2.5 py-0.5 text-xs text-muted-foreground">{matches.length}</span>
        </div>
        {matches.length === 0 ? (
          <EmptyState icon={Swords} title="Sin partidos jugados" description="Este equipo aún no ha disputado partidos finalizados." />
        ) : (
          <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
            <Table>
              <TableHeader>
                  ...
                  return (
                    <TableRow key={match.id} className="transition-colors hover:bg-muted/40">
                      <TableCell className="text-center">
                        <span className={cn("inline-flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold", resultBadge(result))}>
                          {resultLabel[result]}
                        </span>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="hidden text-sm text-muted-foreground sm:table-cell">
                        {match.date.toLocaleDateString("es-AR", { day: "numeric", month: "short", year: "numeric" })}
                      </TableCell>
                      <TableCell className="hidden text-center text-sm text-muted-foreground md:table-cell">{match.round}</TableCell>
                    </TableRow>
                  )
                })}
              </TableBody>
      ...
      <section>
        <div className="mb-4 flex items-center gap-2">
          ...
          <span className="rounded-full bg-muted px-2.5 py-0.5 text-xs text-muted-foreground">{topScorersRaw.length}</span>
        </div>
        {topScorersRaw.length === 0 ? (
          <EmptyState icon={Goal} title="Sin goles registrados" description="Este equipo aún no ha marcado goles en partidos finalizados." />
        ) : (
          <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {topScorersRaw.map((entry, idx) => {
                  ...
                  return (
                    <TableRow key={entry.playerId} className="transition-colors hover:bg-muted/40">
                      <TableCell className="text-center text-sm text-muted-foreground">{idx + 1}</TableCell>
                      <TableCell className="font-medium">
                        <Link href={`/liga/${slug}/jugadores/${entry.playerId}`} className="transition-colors hover:text-primary hover:underline">
      ...
      <section>
        <div className="mb-4 flex items-center gap-2">
          ...
          <span className="rounded-full bg-muted px-2.5 py-0.5 text-xs text-muted-foreground">{cardsByPlayer.length}</span>
        </div>
        {cardsByPlayer.length === 0 ? (
          <EmptyState icon={AlertTriangle} title="Sin tarjetas" description="Este equipo no ha recibido tarjetas en partidos finalizados." />
        ) : (
          <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50">
                  <TableHead className="w-12 text-center">#</TableHead>
                  <TableHead>Jugador</TableHead>
                  <TableHead className="w-24 text-center"><span className="inline-flex items-center gap-1.5"><span className="inline-block h-3 w-2 rounded-sm bg-yellow-400" />Amarillas</span></TableHead>
                  <TableHead className="w-24 text-center"><span className="inline-flex items-center gap-1.5"><span className="inline-block h-3 w-2 rounded-sm bg-red-500" />Rojas</span></TableHead>
                  ...
                  .map((entry, idx) => {
                    ...
                    return (
                      <TableRow key={entry.playerId} className="transition-colors hover:bg-muted/40">
                        ...
                        <TableCell className="text-center"><span className="inline-flex items-center justify-center gap-1 rounded-full bg-yellow-100 px-3 py-1 text-sm font-bold text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400"><span className="inline-block h-2 w-1.5 rounded-sm bg-yellow-400" />{yellows}</span></TableCell>
                        <TableCell className="text-center"><span className="inline-flex items-center justify-center gap-1 rounded-full bg-red-100 px-3 py-1 text-sm font-bold text-red-700 dark:bg-red-900/30 dark:text-red-400"><span className="inline-block h-2 w-1.5 rounded-sm bg-red-500" />{reds}</span></TableCell>
                        <TableCell className="text-center font-semibold tabular-nums">{yellows + reds}</TableCell>
                      </TableRow>
                    )
                  })}
              </TableBody>
      ...
      {categoryTeams.length > 0 && (
        <section>
          <div className="mb-4 flex items-center gap-2">
            <Swords className="h-5 w-5 text-primary" />
            <h2 className="font-heading text-xl font-semibold tracking-tight">Enfrentamientos directos</h2>
          </div>
          <Card size="sm">
            <CardContent className="py-4">
              <p className="mb-3 text-sm text-muted-foreground">
                Seleccioná un rival para ver el historial completo de enfrentamientos.
              </p>
              <TeamVsSelector teams={categoryTeams} currentTeamId={id} />
            </CardContent>
          </Card>
        </section>
      )}
