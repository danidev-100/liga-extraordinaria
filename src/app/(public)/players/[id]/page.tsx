...
export default async function PlayerProfilePage({ params }: PageProps) {
  ...
  // Fetch player with team, goals, and cards (finished matches only)
  ...
  return (
    <div className="space-y-8">
      {/* Back link */}
      <Link
        href={player.team ? `/standings` : "/goleadores"}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Card size="sm" className="border-yellow-400/20 bg-yellow-50/50 dark:bg-yellow-900/10">
          <CardContent className="flex flex-col items-center px-3 py-4 text-center">
            ...
            <div className="mt-0.5 text-2xl font-bold text-yellow-600 dark:text-yellow-400">
              {totalYellow}
            </div>
          </CardContent>
        </Card>
        <Card size="sm" className="border-red-500/20 bg-red-50/50 dark:bg-red-900/10">
          <CardContent className="flex flex-col items-center px-3 py-4 text-center">
            ...
            <div className="mt-0.5 text-2xl font-bold text-red-600 dark:text-red-400">
              {totalRed}
            </div>
          </CardContent>
        </Card>
      ...
      <section>
        ...
        {player.goals.length === 0 ? (
          <EmptyState
            ...
            description="Este jugador aún no ha marcado goles en partidos finalizados."
          />
        ) : (
              ...
              <TableBody>
                {player.goals.map((goal) => {
                  const opponent = resolveOpponent(
                    goal.match.localTeamId,
                    goal.match.visitorTeamId,
                    goal.match.localTeam,
                    goal.match.visitorTeam,
                  )
                  return (
                    <TableRow key={goal.id} className="transition-colors hover:bg-muted/40">
                      <TableCell className="hidden text-sm text-muted-foreground sm:table-cell">
                        {formatDate(goal.match.date)}
                      </TableCell>
                      <TableCell>
      ...
      <section>
        ...
        {player.cards.length === 0 ? (
          <EmptyState
            icon={ShieldAlert}
            title="Sin tarjetas"
            description="Este jugador no ha recibido tarjetas en partidos finalizados."
          />
        ) : (
          <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
            <Table>
              ...
              <TableBody>
                {player.cards.map((card) => {
                  const opponent = resolveOpponent(
                    card.match.localTeamId,
                    card.match.visitorTeamId,
                    card.match.localTeam,
                    card.match.visitorTeam,
                  )
                  return (
                    <TableRow key={card.id} className="transition-colors hover:bg-muted/40">
                      <TableCell className="hidden text-sm text-muted-foreground sm:table-cell">
                        {formatDate(card.match.date)}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <TeamLogo logoUrl={opponent.logoUrl} color={opponent.color} name={opponent.name} size="md" />
                      ...
                      <TableCell className="text-center">
                        {card.type === "YELLOW" ? (
                          <Badge
                            variant="outline"
                            className="border-yellow-400 bg-yellow-50 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400"
                          >
                            <span className="mr-1 inline-block h-2 w-1.5 rounded-sm bg-yellow-400" />
                            Amarilla
                          </Badge>
                        ) : (
                          <Badge
                          ...
                          >
                            <span className="mr-1 inline-block h-2 w-1.5 rounded-sm bg-red-500" />
                            Roja
                          </Badge>
                        )}
                      </TableCell>
